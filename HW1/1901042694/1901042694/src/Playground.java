    ...
    public void setPosition(int Position) {
        ...
        if(getCheckPosition()==0){
            ...
            for(i=0;i<getOurStreet().getUsed();++i){
                if(this.getSide()==getOurStreet().getData()[i].getSide()){
                    if(this.equals(getOurStreet().getData()[i])){
                        if(i+1!=getOurStreet().getUsed()){
                            if(Position<=getOurStreet().getData()[i+1].position&&Position+this.getLength()<=getOurStreet().getData()[i].position){
                                flag=true;
                                break;
                            }
                        }else{
                ...
                if(getOurStreet().getUsed()==0){super.setLength(length);}
                    ...
                    for(i=0;i<getOurStreet().getUsed();++i){
                        if(this.getSide()==getOurStreet().getData()[i].getSide()){
                            ...
                            int first=getOurStreet().getData()[i].position+getOurStreet().getData()[i].getLength();
                            try {
                                if(getOurStreet().getData()[i+1].equals(this)){
                                    if(i+2==getOurStreet().used){
                                        ...
                                        if(this.getSide()==getOurStreet().getData()[i+2].getSide()&&this.position+length<=getOurStreet().getData()[i+2].position){
                                            flag=true;
                                            break;
                                        }
                                    }
